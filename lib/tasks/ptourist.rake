namespace :ptourist do
  ...
  task delete_subjects: :environment do
    ...
    DatabaseCleaner[:active_record].clean_with(:truncation, {:except=>%w[users roles]})
  ...
  task delete_users: :environment do
    ...
    DatabaseCleaner[:active_record].clean_with(:truncation, {:only=>%w[users roles]})
  end

  desc "reset users"
  task users: [:delete_users] do
  ...
  task subjects: [:delete_subjects] do
